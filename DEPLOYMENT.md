...
- **Telegram — PUBLIC channels (Phase 3) — BUILT, env-activated.** Set `TELEGRAM_API_ID` + `TELEGRAM_API_HASH` + `TELEGRAM_SESSION` (an MTProto **user** session — a bot can't read channel history) + `TELEGRAM_CHANNELS` (comma-separated allow-list) and the connector (`lib/sourcing/connectors/telegram/`) joins every search; absent, it is omitted. It reads the allow-listed public job/CV channels, drops posts older than `TELEGRAM_MAX_AGE_DAYS` (default 45), collapses reposts to the latest, drops "found a job / vacancy closed" posts, and classifies each surviving post with Gemini Flash — surfacing only real candidate CVs (recruiter **vacancies are rejected**), with the @handle/phone captured for promotion. Tuning: `TELEGRAM_PER_CHANNEL_LIMIT` (default 200). Distinct from `TELEGRAM_BOT_TOKEN` (inbox notifications). **Needs a real user session string + a working Gemini key before a live run.** See `SOURCING_RUNBOOK.md` §C2.
- **Telegram — OWNED channels via BOT (Phase 3b) — BUILT.** Migration `20260420000340_telegram_bot_intake.sql`. For a company's **own** CV-intake channel: add the existing `TELEGRAM_BOT_TOKEN` bot as a channel **admin**, register the handle (`POST /api/hr/telegram-channels`), and the `telegram-bot-ingest` cron (every 2 min, route `/api/internal/telegram/ingest`, service-role bearer) polls `getUpdates`, stages each post into `telegram_posts`, and classifies it **once** at ingest. The funnel's DB-backed reader then surfaces candidates with **no MTProto session and no per-search Gemini cost**. A channel handle is globally unique (one tenant per channel). Freshness/retention is `TELEGRAM_INTAKE_RETENTION_DAYS` (default 90); the `telegram-posts-purge` cron (`purge_stale_telegram_posts`, nightly 03:50) prunes the pool. New Vault secrets: none (reuses `app_url` + `service_role_key` from migration 330). Only live prerequisite: a working **Gemini key**. See `SOURCING_RUNBOOK.md` §C2b.
