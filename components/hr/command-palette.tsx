    ...
    <DialogContent className="p-0 sm:max-w-xl">
      ...
      <div className="flex items-center gap-2 border-b border-[var(--color-rule-2)] px-4">
        ...
        <input
          ...
          className="h-12 flex-1 bg-transparent text-sm text-[var(--color-ink)] outline-none placeholder:text-[var(--color-ink-5)]"
          aria-label={t("hr.palette.title")}
          aria-autocomplete="list"
          aria-activedescendant={activeItem ? itemDomId(activeItem) : undefined}
        />
        {loading && (
          <span className="text-[11px] text-[var(--color-ink-4)]">{t("hr.palette.loading")}</span>
        )}
      </div>
...
function JobRow({
  ...
  return (
    <Row id={itemDomId(item)} active={active} onClick={(meta) => onActivate(item, meta)}>
      <Briefcase className="h-4 w-4 text-[var(--color-ink-4)]" />
...
function CandidateRow({
  ...
  return (
    <Row id={itemDomId(item)} active={active} onClick={(meta) => onActivate(item, meta)}>
      <UserRound className="h-4 w-4 text-[var(--color-ink-4)]" />
      ...
      {item.matchScore != null && (
        <span className="rounded-[var(--radius-sm)] bg-[var(--color-persimmon-tint)] px-1.5 py-0.5 text-[10px] font-[var(--font-tez-mono)] font-semibold text-[var(--color-persimmon-2)]">
          {item.matchScore}
        </span>
      )}
    </Row>
